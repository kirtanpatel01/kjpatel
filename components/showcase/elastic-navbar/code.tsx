...
function HomePage() {
  const cards = [
    {
      ...
      desc: "Why use 5 lines of CSS when you can use a 50MB library?",
    },
    {
      title: "Blazing Fast",
      desc: "It loads so fast you won't even have time to reconsider your life choices.",
    },
  ];
  ...
  return (
    <PageWrapper>
      ...
      <h1 className="text-2xl font-normal underline decoration-dashed underline-offset-8 decoration-lime-500/50">
        This is the Home Page
      </h1>
      <p className="mt-6 text-muted-foreground text-lg font-normal max-w-lg leading-relaxed">
        I spent hours on this site just for you to look at a single sentence. I
        hope you&apos;re happy.
      </p>
      <div className="mt-8 grid grid-cols-1 md:grid-cols-2 gap-4 w-full max-w-xl">
        {cards.map((card, i) => (
          <div
            key={i}
            className="p-4 border border-dashed rounded text-left bg-accent/10"
          >
            <p className="text-base font-normal underline decoration-dashed underline-offset-2 decoration-lime-500/30">
              {card.title}
            </p>
...
function AboutPage() {
  ...
  return (
    <PageWrapper>
      <h1 className="text-2xl font-normal italic opacity-80">
        A Little About Me
      </h1>
      <p className="mt-6 text-muted-foreground text-lg font-normal max-w-lg leading-relaxed">
        I enjoy taking credit for things I found on Stack Overflow and talking
        about my passion for learning while actually just binging Netflix.
      </p>
      <div className="mt-10 flex flex-col gap-4 w-full max-w-lg">
        {facts.map((fact, i) => (
          <div
            key={i}
            className="p-4 border border-dashed rounded bg-accent/20 text-base text-muted-foreground font-normal"
          >
            &ldquo;{fact}&rdquo;
          </div>
        ))}
  ...
  return (
    <PageWrapper>
      ...
      <p className="mt-4 text-muted-foreground text-lg font-normal max-w-lg leading-relaxed">
        Here is a list of all the things I started and immediately forgot about.
        The list is empty because I haven&apos;t even finished the list itself.
      </p>
      <div className="mt-12 space-y-4 w-full max-w-xl text-left">
        {projects.map((p, i) => (
          <div
            key={i}
            className="flex justify-between items-baseline p-4 border-b border-dashed border-border/50"
          >
            <span className="text-base font-normal">{p.name}</span>
            <span className="text-base text-muted-foreground italic  opacity-60 underline decoration-dashed decoration-1 underline-offset-4">
              {p.status}
            </span>
          </div>
        ))}
...
function SkillsPage() {
  ...
  return (
    <PageWrapper>
      ...
      <p className="mt-6 text-muted-foreground text-lg font-normal max-w-lg leading-relaxed">
        I&apos;m very good at typing things into search bars and nodding my head
        pretending to know what a recursive function is.
      </p>
      <div className="mt-10 flex flex-wrap justify-center gap-4 max-w-xl">
        {skills.map((s, i) => (
          <div
            key={i}
            className="px-5 py-2 border border-dashed rounded text-base text-muted-foreground font-normal bg-accent/10"
          >
            {s}
          </div>
        ))}
...
function ContactPage() {
  return (
    <PageWrapper>
      ...
      <p className="mt-6 text-muted-foreground text-lg font-normal max-w-lg leading-relaxed">
        Go ahead and send me an email. I&apos;ll archive it as soon as it lands
        so my inbox stays clean.
      </p>
      <div className="mt-12 w-full max-w-sm space-y-6">
        <input
          className="w-full p-4 border-b border-dashed bg-transparent text-base outline-none focus:border-lime-500 transition-colors"
