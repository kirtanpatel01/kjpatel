...
function TwinklingStar({ data }: { data: StarData }) {
  ...
  useFrame(({ clock }) => {
    ...
    const pulse =
      (Math.sin(t * data.twinkleSpeed + data.twinklePhase) + 1) * 0.5;
    ...
    if (spikeARef.current) {
      spikeARef.current.scale.set(
        1 + pulse * 0.8,
        data.spikeScale + pulse * 0.5,
        1,
      );
      (spikeARef.current.material as THREE.MeshBasicMaterial).opacity =
        opacity * 0.65;
    }

    if (spikeBRef.current) {
      spikeBRef.current.scale.set(
        1 + pulse * 0.8,
        data.spikeScale * 0.85 + pulse * 0.45,
        1,
      );
      (spikeBRef.current.material as THREE.MeshBasicMaterial).opacity =
        opacity * 0.55;
    }
  });
