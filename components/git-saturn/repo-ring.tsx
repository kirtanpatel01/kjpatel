...
import { Meteoroid } from "@/components/git-saturn/git-meteoroid";
...
function sizeFromCommits(
  ...
  const curved = normalized ** 0.72;
  return 0.02 + curved * 0.062;
}

function getRepoPose(
  index: number,
  total: number,
  ringRadius: number,
  repoCommits: number,
  ...
  const verticalOffset =
    (seeded(index * 2.33 + repoCommits * 0.13) - 0.5) * 0.04;
...
function CommitBurst({
  ...
  const normalized =
    maxCommits > minCommits
      ? (repo.commits - minCommits) / (maxCommits - minCommits)
      : 0;
  const burstStrength = Math.max(0, normalized - 0.35);
  const burstEnabled = burstStrength > 0;

  const pose = React.useMemo(
    () => getRepoPose(index, total, ringRadius, repo.commits),
    [index, repo.commits, ringRadius, total],
  );
  ...
  useFrame((_, delta) => {
    ...
    if (!state.active) {
      ...
      if (state.delay <= 0 && frontArc > 0.35) {
        ...
        state.delay =
          0.9 + (1 - burstStrength) * 1.8 + seeded(index * 8.1) * 0.8;
        mesh.visible = true;
        ...
        state.velocity.set(
          tangentX * (0.12 + burstStrength * 0.16),
          0.01 + burstStrength * 0.015,
          tangentZ * (0.12 + burstStrength * 0.16),
        );
      }
...
export function RepoRing({
  ...
  const minCommits = repos.reduce(
    (min, repo) => Math.min(min, repo.commits),
    Number.POSITIVE_INFINITY,
  );
  const maxCommits = repos.reduce(
    (max, repo) => Math.max(max, repo.commits),
    0,
  );
